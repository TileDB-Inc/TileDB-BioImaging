...
def image_list_to_tiles(
    ...
    for slide_num in image_num_list:
        tile_summary = summary_and_tiles(
            slide_num, display, save_summary, save_data, save_top_tiles
        )
        tile_summaries_dict[slide_num] = tile_summary
    return image_num_list, tile_summaries_dict


def image_range_to_tiles(
    start_ind,
    end_ind,
    display=False,
    save_summary=True,
...
def singleprocess_filtered_images_to_tiles(
    display=False,
    save_summary=True,
    save_data=True,
    save_top_tiles=True,
...
def multiprocess_filtered_images_to_tiles(
    display=False,
    save_summary=True,
    save_data=True,
    save_top_tiles=True,
    ...
    if image_num_list is not None:
        ...
        # HACK hardcode this b/c only ever processing 1 here
        num_train_images = slide.get_num_training_slides()
    ...
    for num_process in range(1, num_processes + 1):
        ...
        if image_num_list is not None:
            ...
            if start_index == end_index:
                print(
                    "Task #" + str(num_process) + ": Process slide " + str(start_index)
                )
            else:
                print(
                    "Task #"
...
def image_row(slide_num, tile_summary, data_link):
    ...
    if data_link:
        html += (
            '        <div style="white-space: nowrap;">S%03d Tile Summary\n' % slide_num
...
def generate_tiled_html_result(slide_nums, tile_summaries_dict, data_link):
    ...
    if not slide.TILE_SUMMARY_PAGINATE:
        ...
        for page_num in range(1, num_pages + 1):
            ...
            if page_num == 1:
                text_file = open(
                    os.path.join(slide.TILE_SUMMARY_HTML_DIR, "tiles.html"), "w"
                )
            else:
                text_file = open(
                    os.path.join(
                        slide.TILE_SUMMARY_HTML_DIR, "tiles-%d.html" % page_num
                    ),
                    "w",
                )
            text_file.write(html)
...
def dynamic_tiles(slide_num, small_tile_in_tile=False):
    ...
    tile_summary = score_tiles(
        slide_num, filt_np_img, (large_w, large_h, small_w, small_h), small_tile_in_tile
    )
    return tile_summary
