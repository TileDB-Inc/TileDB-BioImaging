...
def test_ome_tiff():
    ...
    Reference_Schema.append(tiledb.ArraySchema(domain=tiledb.Domain(*[
            tiledb.Dim(name='X', domain=(0, 2219), tile=1024, dtype=np.uint64),
            tiledb.Dim(name='Y', domain=(0, 2966), tile=1024, dtype=np.uint64),
        ]), sparse=False,
        attrs=[
            tiledb.Attr(name='rgb', dtype=[('f0', 'u1'), ('f1', 'u1'), ('f2', 'u1')], var=False, nullable=False, filters=tiledb.FilterList([tiledb.ZstdFilter(level=0)]))
        ], cell_order= 'row-major', tile_order='row-major', capacity=10000
    ))
    ...
    Reference_Schema.append(tiledb.ArraySchema(domain=tiledb.Domain(*[
        ...
        attrs=[
            tiledb.Attr(name='rgb', dtype=[('f0', 'u1'), ('f1', 'u1'), ('f2', 'u1')], var=False, nullable=False, filters=tiledb.FilterList([tiledb.ZstdFilter(level=1)]))
        ], cell_order= 'row-major', tile_order='row-major', capacity=10000
    ))
    ...
    Reference_Schema.append(tiledb.ArraySchema(domain=tiledb.Domain(*[
        ...
        attrs=[
            tiledb.Attr(name='rgb', dtype=[('f0', 'u1'), ('f1', 'u1'), ('f2', 'u1')], var=False, nullable=False, filters=tiledb.FilterList([tiledb.ZstdFilter(level=2)]))
        ], cell_order= 'row-major', tile_order='row-major', capacity=10000
    ))
    ...
    assert (
        ...
        and t.level_info[1] == LevelInfo(abspath = '', level = 1, schema = Reference_Schema[1])
        and t.level_info[2] == LevelInfo(abspath = '', level = 2, schema = Reference_Schema[2])
    )
