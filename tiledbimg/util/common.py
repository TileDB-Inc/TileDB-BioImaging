...
class ImageConverter(object):
    ...
    def level_tile(self, shape):
        level_tile = LevelTile(
            np.min((self.tile_x, shape[0])),
            np.min((self.tile_y, shape[1]))
        )
        return level_tile
    ...
    def create_schema(self, img_shape):
        ...
        dims.append(
            tiledb.Dim(name="X", domain=(0,img_shape[0]-1), dtype=np.uint64, tile=level_tile.x)
        )
        dims.append(
                tiledb.Dim(name="Y", domain=(0,img_shape[1]-1), dtype=np.uint64, tile=level_tile.y)
        )
